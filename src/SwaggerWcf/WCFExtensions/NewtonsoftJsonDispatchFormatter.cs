...
namespace SwaggerWcf
{
    public class NewtonsoftJsonDispatchFormatter : IDispatchMessageFormatter
    {
        ...
        public NewtonsoftJsonDispatchFormatter(OperationDescription operationDescription, bool isRequest)
        {
            this.operation = operationDescription;
            if (isRequest)
            {

                int operationParameterCount = operationDescription?.Messages?.FirstOrDefault()?.Body?.Parts?.Count ?? 0;
